...
public:
        ...
        cm_id(_cm_id)
    {
        ...
        if (IsGpuMemory(buffer)) {
            ...
            std::cout << "RdmaMemoryRegion: Registering system memory" << std::endl;
...
private:
    ...
    ibv_mr* RegisterGpuMemory(void* gpuBuffer, size_t length)
    {
        ...
        if (access("/sys/module/nvidia_peermem", F_OK) == 0) {
            std::cout << "RdmaMemoryRegion: Registering GPU Direct RDMA memory" << std::endl;
        ...
        std::cout << "RdmaMemoryRegion: GPU Direct not available, using standard registration" << std::endl;
        return rdma_reg_msgs(cm_id, gpuBuffer, length);
    }
