...
int main(int argc, char *argv[])
{
    ...
    int msg_count = 0;
    while (true)  // Infinite loop - will break on timeout or error
    {
        ...
        if (result != easyrdma_Error_Success)
        {
            ...
            {
                std::cout << "No more data received (timeout) - client likely finished" << std::endl;
                break;
            }
            else if (result == easyrdma_Error_Disconnected)
            {
                std::cout << "Client disconnected - session ended normally" << std::endl;
                break;
            }
            else
            {
        ...
        std::cout << "Message " << msg_count << ": \"" << message << "\" (" << receive_region.usedSize << " bytes)" << std::endl;
    ...
    std::cout << "Total messages processed: " << msg_count << std::endl;
    easyrdma_CloseSession(connected_session);
    easyrdma_CloseSession(listen_session);
