...
void send_completion_callback(void* context1, void* context2, int32_t status, size_t bytes) {
    if (status == easyrdma_Error_Success) {
        std::cout << "Host data sent successfully!" << std::endl;
    } else {
...
int main(int argc, char *argv[])
{
    ...
    const char *message = "GPU Direct RDMA Send: External Host Memory → GPU!";
    strcpy(hostBuffer, message);
    int messageLength = strlen(message) + 1;
    std::cout << "Host buffer allocated and initialized for sending" << std::endl;


    result = easyrdma_ConfigureExternalBuffer(client_session, hostBuffer, buffer_size, 1);
    if (result != easyrdma_Error_Success)
    {
    ...
    std::cout << "Configured session to use external Host memory buffer" << std::endl;
    ...
    callbackData.context1 = hostBuffer;
    result = easyrdma_QueueExternalBufferRegion(client_session, hostBuffer, buffer_size, &callbackData, 10000);
    if (result != easyrdma_Error_Success)
    {
