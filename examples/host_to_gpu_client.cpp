...
int main(int argc, char *argv[])
{
    ...
    // Copy message into the acquired buffer
    const char *message = "GPU Direct RDMA WRITE: Host→GPU Direct!";
    int messageLength = strlen(message) + 1;
    memcpy(send_region.buffer, message, messageLength);
    ...
    std::cout << "Message: \"" << (char*)send_region.buffer << "\" (" << messageLength << " bytes)" << std::endl;
